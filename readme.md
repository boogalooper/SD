...
* записать скрипт в экшен и активировать значок запуска с диалоговым окном
  ![](assets/160057.png)
* использовать файл сценария [SD API forwarding.vbs](SD%API%forwarding.vbs) чтобы запустить скрипт из интерфейса операционной системы (ручной запуск, запуск с помощью хоткеев и т.п.). Этот вспомогательный файл запускает сценарий Windows передающий Фотошопу команду на запуск скрипта. Диалоговый режим активируется в том, если файлу `SD API forwarding.vbs`передан любой аргумент командной строки.
